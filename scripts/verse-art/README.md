# Verse Art collection

The app bundles **180 finished WebP images and 180 thumbnails**, organized into six disjoint collections of 30: Hope, Peace, Strength, Joy, Comfort, and Gratitude. No network request or image-generation service is needed in the app. Images can be saved and shared offline.
...
- `android/app/src/main/assets/verse-art/`: the finished images used by previews, saving, and sharing.
...
4. Review the six contact sheets and representative full-size cards. Copy the generated WebPs into the main `verse-art` assets directory and the contact sheets into `assets/verse-art/rendered/`.
...
Verse Art uses its own gallery icon in the drawer; Remove Ads retains its sparkle icon. Shared images are refreshed from the current bundled artwork, so previously cached images cannot survive an artwork update.

The native screen follows the existing app theme and appears under **Navigation drawer → Verse Art → Category**. Favorites reuse the existing verse favorite IDs and backup behavior. Art remains English WEB independently of the reading translation, which is stated on its detail screen.
